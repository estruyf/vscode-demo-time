...
export class SlideScreenshotService {
  ...
  public static async getNextSlideScreenshot(): Promise<string | null> {
    try {
      ...
      if (nextDemo?.id === SlideScreenshotService.lastDemoId && SlideScreenshotService.cachedScreenshot) {
        Logger.info('Returning cached next slide screenshot');
        return SlideScreenshotService.cachedScreenshot;
      }
      ...
      const slideStep = nextDemo.steps.find((step) => step.action === Action.OpenSlide && step.path);
      ...
      const slideIndex = typeof slideStep.slide === 'number' ? slideStep.slide : 0;
      const targetSlide = slides[slideIndex] || slides[0];

      // Generate screenshot
      const screenshot = await SlideScreenshotService.generateScreenshot(targetSlide);

      // Cache the result
      SlideScreenshotService.cachedScreenshot = screenshot;
      SlideScreenshotService.lastDemoId = nextDemo.id;
  ...
  public static clearCache(): void {
    SlideScreenshotService.cachedScreenshot = null;
    SlideScreenshotService.lastDemoId = undefined;
  }
  ...
  private static async generateScreenshot(slide: any): Promise<string> {
    const { chromium } = await SlideScreenshotService.getPlaywright();
    ...
    try {
      ...
      const html = await SlideScreenshotService.generateSlideHtml(slide);
...
<body>
  ...
  private static async getPlaywright(): Promise<typeof import('playwright-chromium')> {
    ...
    try {
      return await import(
        await resolve('playwright-chromium', { url: wsFolder?.uri.fsPath })
      );
    } catch {}
