...
// Type the mocked ScriptExecutor
const mockScriptExecutor = ScriptExecutor as jest.Mocked<typeof ScriptExecutor>;
...
describe('MacOSActionsService', () => {
  ...
  describe('Volume Control', () => {
    it('should mute volume on macOS', async () => {
      ...
      mockScriptExecutor.executeScriptAsync.mockResolvedValue('');
    ...
    it('should show error when muting volume fails', async () => {
      ...
      mockScriptExecutor.executeScriptAsync.mockRejectedValue(errorMessage);
  ...
  describe('Caffeine (Sleep Prevention)', () => {
    ...
    it('should enable caffeine indefinitely on macOS', async () => {
      (os.platform as jest.Mock).mockReturnValue('darwin');
      mockScriptExecutor.executeScriptAsync.mockResolvedValue('12345\n');
    ...
    it('should disable caffeine on macOS', async () => {
      (os.platform as jest.Mock).mockReturnValue('darwin');
      mockScriptExecutor.executeScriptAsync
    ...
    it('should handle disabling caffeine when no process is running', async () => {
      (os.platform as jest.Mock).mockReturnValue('darwin');
      const errorMessage = 'No matching processes';
      mockScriptExecutor.executeScriptAsync.mockRejectedValueOnce(errorMessage);
    ...
    it('should show error when enabling caffeine fails', async () => {
      (os.platform as jest.Mock).mockReturnValue('darwin');
      const errorMessage = 'Failed to start caffeinate';
      mockScriptExecutor.executeScriptAsync.mockRejectedValue(errorMessage);
