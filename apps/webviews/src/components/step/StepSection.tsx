...
import { Plus, Play, Square, List, Info } from 'lucide-react';
      ...
      {showInfo && (
        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-md">
          <p className="text-xs text-green-700">
            <span className="font-medium text-green-800">About Steps:</span> A step defines an action that you want to perform for your demo. Each demo can have multiple steps.
      ...
      <div className="space-y-3">
        {demo.steps.map((step, index) => (
          <StepListItem
            ...
            onEdit={() => onEditStep(editingStep?.stepIndex === index ? null : index)}
            onRemove={() => onRemoveStep(index)}
            onDuplicate={() => onDuplicateStep(index)}
            onReorder={handleReorder}
            draggingIndex={draggingIndex}
            setDraggingIndex={setDraggingIndex}
            editor={editingStep?.stepIndex === index && editingStep?.demoIndex === selectedDemo ? (
              <StepEditor
                step={step}
                onChange={(updatedStep) => onStepChange(index, updatedStep)} />
            ) : undefined} onCloseEdit={function (): void {
        ...
        {demo.steps.length === 0 && (
          <div className="text-center py-12 text-gray-500 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
            ...
            <h3 className="text-lg font-medium text-gray-800">No Steps Yet</h3>
