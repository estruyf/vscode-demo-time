...
export const getActionIcon = (action: Action): string => {
  const iconMap: Record<Action, string> = {
    ...
    [Action.SendKeybinding]: 'keyboard',
    ...
    [Action.HideDock]: 'panel-bottom-close',
    [Action.ShowDock]: 'panel-bottom-open',
    [Action.HideDesktopIcons]: 'layout-sidebar-left-off',
    [Action.ShowDesktopIcons]: 'layout-sidebar-left',
...
export const getFieldsForAction = (action: Action): string[] => {
  const fieldMap: Record<Action, string[]> = {
    ...
    [Action.FocusTerminal]: ['terminalId'],
