...
Focus the terminal view in the editor.

<Tabs syncKey="focus-terminal">
<TabItem label="JSON">
...
- **insertTypingSpeedRandomness**: Optional. Adds a semi-random variation to the `character-by-character` typing speed so it feels less robotic. The value is the maximum percentage (`0`-`100`) of extra delay added on top of `insertTypingSpeed` for each character. Defaults to `0` (disabled).
...
## Send a keyboard shortcut to the terminal

Use the `sendKeybinding` action for keyboard shortcuts. If you want the shortcut to land in the terminal, focus the terminal first and then send the keybinding.

<Tabs syncKey="terminal-keybinding-example">
<TabItem label="JSON">
...
- **id**: The ID of the script, used to reference its output in later steps
- **path**: Path to the script file to execute
- **command**: The command/runtime to use (e.g., `node`, `bash`, `python`, `powershell`)
- **args**: Optional. Array of strings passed as positional command-line arguments to the script
- **waitForMessage**: Optional. When set, the step waits until the script outputs this string before advancing, rather than waiting for the process to exit. Useful for long-running scripts that signal readiness (e.g., `"done"` or `"Server ready"`)
- **showProgress**: Optional. Whether to show the progress notification while the script is executing. Default is `true`
...
console.log('First arg:', args[0]);  // First argument
