...
<Aside type="tip">
You can click on the **demo** or **demo step** to open the corresponding configuration in the Act Editor.
</Aside>
...
<Aside type="note">
Notice the `steps` array in the demo. Each move will be executed in sequence when you trigger the demo. In this example, when the `Create and open sample.json` scene is triggered, it creates a `sample.json` file with the specified content and then opens it in the editor all in one go.
</Aside>
...
<Aside type="note">
If you want to split your demos in multiple sections, you can do this by creating multiple act files in the `.demo` folder. The extension will automatically recognize them and display them in the **Demo Time** view.
</Aside>
