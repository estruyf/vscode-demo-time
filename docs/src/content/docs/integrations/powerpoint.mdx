...
2. **Add the Add-in to Your Presentation**  
   ...
   - The initial view from the add-in will ask you for the `Demo Time - API URL` and `Scene ID`
      - The `Demo Time - API URL` is the URL of the Demo Time API, which is by default `http://localhost:3710`.
      - The `Scene ID` is the ID of the command you want to trigger from your script.
      ...
      - You can test out the scene ID by clicking on the `test` button.
