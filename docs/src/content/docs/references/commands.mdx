...
lastmod: 2025-08-22T10:00:43.146Z
---
...
| `Demo Time: Create a new act file` | Create a new act file. |
| `Demo Time: Start` | Starts the scene or runs the next scene. |
| `Demo Time: Previous` | Go back to the previous scene (only in presentation mode and when the `demoTime.previousEnabled` setting is enabled). |
| `Demo Time: Add move` | Add the current selection as a move to the act file. |
| `Demo Time: Reset` | Reset the whole play (all acts and scenes will be reset). |
...
| `Demo Time: Add move to scene` | Add a new scene. |
| `Demo Time: Toggle highlight` | Toggle the highlight on and off when the current scene includes a highlight action. |
...
| `Demo Time: Record demo` | Start recording your actions into a new act file. See [Recording demos](/features/recording-demos/). |
| `Demo Time: Stop recording` | Stop recording and create the act file from the captured scenes and moves. |
| `Demo Time: Mark highlight` | *(while recording)* Capture the current selection as a `highlight` move. |
| `Demo Time: Split move` | *(while recording)* End the current move and start a new one. |
| `Demo Time: New scene` | *(while recording)* End the current scene and start a new one. |
...
| `Demo Time: Settings` | Open the settings for Demo Time. |

## Pro Commands

The following commands are only available to [Pro users](/pro-features/).

| Command | Description |
| --- | --- |
| `Demo Time: Authenticate with GitHub (Unlock Pro Features)` | Authenticate with GitHub to unlock Pro features. |
| `Demo Time Analytics: Start Analytics Recording` | Start recording analytics for your presentation session. |
| `Demo Time Analytics: Stop Analytics Recording` | Stop recording analytics. |
| `Demo Time Analytics: Toggle Analytics Recording` | Toggle analytics recording on/off. |
| `Demo Time Analytics: Open Analytics Dashboard` | Open the analytics dashboard to view your presentation metrics. |
| `Demo Time Analytics: Export Analytics Session` | Export your analytics session data as JSON. |

<Aside type="tip">
Learn more about Pro features and how to unlock them in the [Pro Features documentation](/pro-features/).
</Aside>
