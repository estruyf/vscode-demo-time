...
Highlights are a great way to draw attention to specific code snippets during your presentation. To ensure a clean transition between moves, Demo Time now automatically clears any active highlight when you move to the next move in your scene.
...
When the current scene includes a highlight action, there will be a highlight button available in the editor title bar on the right. You can use this to toggle the highlight on and off.
