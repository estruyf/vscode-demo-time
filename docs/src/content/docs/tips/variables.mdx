...
- `{DT_WORKSPACE_PATH}`: The filesystem path to the current workspace folder.
...
You can use `{DT_WORKSPACE_PATH}` when you want to build a local URL from the current workspace folder.

<Code code={`
{
    "action": "openWebsite",
    "url": "{DT_WORKSPACE_PATH}/Reports/index.html",
    "openInVSCode": true
}
`} 
  lang="json" 
